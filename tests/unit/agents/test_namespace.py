...
class TestResolveKnowledgeNamespace:
    ...
    def test_default_namespace_with_principal(self) -> None:
        ...
        assert resolve_knowledge_namespace(spec, _ALICE) == "agent:system:test-agent:u:alice"

    def test_owner_isolation(self) -> None:
        alice_spec = self._make_spec(owner_id="alice")
        bob_spec = self._make_spec(owner_id="bob")
        assert resolve_knowledge_namespace(alice_spec, _ALICE) != resolve_knowledge_namespace(bob_spec, _ALICE)
...
class TestResolveKnowledgeNamespaceForIdentity:
...
class TestNamespaceIsolation:
    ...
    def test_prefix_isolation(self) -> None:
        ns1 = resolve_knowledge_namespace_for_identity(
            name="bot", owner_id="system", namespace_template=None, principal=_ALICE
        )
        ns2 = resolve_knowledge_namespace_for_identity(
    ...
    def test_user_isolation(self) -> None:
        bob = AuthenticatedPrincipal(id="bob", type="user")
        ns_alice = resolve_knowledge_namespace_for_identity(
            name="bot", owner_id="system", namespace_template=None, principal=_ALICE
        )
        ns_bob = resolve_knowledge_namespace_for_identity(
