...
- **Audit UI viewer** — `/ui/audit` (admin-only) renders the `gateway:audit` Redis stream with live tail + paginated history. Backed by `routes/audit.py` (`GET /api/v1/audit/{status,events,events/stream}`) which reads via `XREVRANGE` / `XREAD` against the `RedisStreamAuditSink`. Admin-gating comes from `GET /api/v1/auth/whoami`; React uses `AdminGuard` + sidebar conditional rendering. Empty state points at `docs/guides/observability.md` when the sink isn't configured.
