...
import { useAgentLineage } from "../hooks/useAgentLineage";

export default function AgentLineage() {
  const { name = "" } = useParams<{ name: string }>();
  const { lineage, loading, error, refresh } = useAgentLineage(name);
  const [selected, setSelected] = useState<AgentVersion | null>(null);
  const [deploying, setDeploying] = useState(false);

  const handleDeploy = async (v: AgentVersion) => {
    setDeploying(true);
    try {
      await api.deployAgentVersion(`${v.owner_id}:${v.agent_name}`, v.version_id);
      await refresh();
    } finally {
      setDeploying(false);
    }
      ...
      {lineage && lineage.nodes.length > 0 && (
        <div className="flex gap-4">
          <div className="flex-1">
            <LineageGraph
              nodes={lineage.nodes.map((n) => ({ kind: "agent", node: n }))}
              deployed={lineage.deployed}
              onSelect={(v) => setSelected(v as AgentVersion)}
          ...
          {selected && (
            <aside className="w-80 shrink-0 rounded border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3 text-xs font-mono max-h-[60vh] overflow-auto">
              ...
              <div className="text-gray-500 dark:text-gray-400 mb-2">
                {selected.owner_id}:{selected.agent_name}
              </div>
