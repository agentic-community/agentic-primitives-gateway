...
import type { TeamSpec, TeamVersion, VersionStatus } from "../api/types";
import LoadingSpinner from "../components/LoadingSpinner";
import { useTeamVersions } from "../hooks/useTeamVersions";
...
export default function TeamVersions() {
  const { name = "" } = useParams<{ name: string }>();
  const [team, setTeam] = useState<TeamSpec | null>(null);
  const { versions, loading, error, refresh } = useTeamVersions(name);
  ...
  useEffect(() => {
    api.getTeam(name).then(setTeam).catch(() => {});
  }, [name]);
  ...
  const deploy = (v: TeamVersion) =>
    run(() => api.deployTeamVersion(name, v.version_id), v.version_id);
  const propose = (v: TeamVersion) =>
    run(() => api.proposeTeamVersion(name, v.version_id), v.version_id);
  const approve = (v: TeamVersion) =>
    run(() => api.approveTeamVersion(name, v.version_id), v.version_id);
  const doReject = async (v: TeamVersion) => {
    if (!rejectReason.trim()) return;
    await run(
      () => api.rejectTeamVersion(name, v.version_id, rejectReason),
      v.version_id,
    );
    setRejectId(null);
    setRejectReason("");
      ...
      <header className="mb-4 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
            Versions of {team?.owner_id ?? "?"}:{name}
          </h1>
        </div>
        <Link
          to={`/teams/${name}/lineage`}
